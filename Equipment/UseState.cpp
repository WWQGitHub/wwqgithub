...
BOOL CUseState::OnInitDialog() 
{
	...
	list.InsertColumn(1,_T("资产使用状况"));//向列表中添加列
...
void CUseState::OnButtonrefresh() 
{
	...
	theApp.datamanage->GetRecordset()->Open("select usestate as 使用状况 from EquipmentManage.dbo.tb_usestate",theApp.datamanage->GetConn().GetInterfacePtr(),adOpenKeyset,adLockPessimistic,adCmdText);
	if (theApp.datamanage->GetRecordset()->RecordCount >0)
	{
